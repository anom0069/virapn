...
if (preg_match('/^\/([Ss]tart)/', $text) or $text == '⤵️ برگرد به منوی اصلی ' or $text == '🔙بازگشت به منوی اصلی' or $data == 'mainMenu') {
    ...
    if(isset($data) and $data == "mainMenu"){
        $res = editText($message_id, 'سلااام به ربات فیلتر شکن پرسرعت ویرا💣🔥 خوش اومدی 🫡🌸
...
', $mainKeys);
        if(!$res->ok){
            sendMessage('سلااام به ربات فیلتر شکن پرسرعت ویرا💣🔥 خوش اومدی 🫡🌸
...
if($data=="inviteSetting" && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        ...
        [
            ...
            ['text'=>"مقدار پورسانت",'callback_data'=>"anomch"]
            ],
        [
...
if($response->success){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ['text'=>"اخیش یکی زمان زد 😁",'callback_data'=>"anomch"]
            ],
        ]]);
sendMessage("
...
if($response->success){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ['text'=>"اخیش یکی حجم زد 😁",'callback_data'=>"anomch"]
            ],
        ]]);
sendMessage("
...
",$keys,"html", $admin);
    ...
    bot('editMessageReplyMarkup',[
		...
		'reply_markup' => json_encode(['inline_keyboard'=>[
		    [['text'=>"پرداخت انجام شد",'callback_data'=>"anomch"]]
		    ]])
    ]);
}else{
...
if ($data == 'message2All' and ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $sendInfo = json_decode(file_get_contents("settings/messageanom.json"),true);
...
if ($userInfo['step'] == 's2a' and $text != $cancelText){
    ...
    file_put_contents("settings/messageanom.json",json_encode($sendInfo));
}
...
\n🌐 subscription : <code>$subLink</code>";
    ...
    unset($markup[count($markup)-1]);
    $markup[] = [['text'=>"✅",'callback_data'=>"anomch"]];
    $keys = json_encode(['inline_keyboard'=>array_values($markup)],488);
    ...
    if($admin != $from_id){ 
        $keys = json_encode(['inline_keyboard'=>[
            [
                ['text'=>"به به 🛍",'callback_data'=>"anomch"]
            ],
            ]]);
        sendMessage("
👨‍👦‍👦 خرید ( زیر مجموعه )
...
if($data=="availableServers"){
    ...
    $keys = array();
    $keys[] = [
        ['text'=>"تعداد باقیمانده",'callback_data'=>"anomch"],
        ['text'=>"پلن",'callback_data'=>"anomch"],
        ['text'=>'سرور','callback_data'=>"anomch"]
        ];
    while($file_detail = $serversList->fetch_assoc()){
        ...
        if($name->num_rows>0){
            ...
            $keys[] = [
                ['text'=>$acount . " اکانت",'callback_data'=>"anomch"],
...
if($data=="availableServers2"){
    ...
    while($file_detail2 = $serversList->fetch_assoc()){
        ...
        if($name->num_rows>0){
            ...
            $keys[] = [
                ['text'=>$acount2 . " اکانت",'callback_data'=>"anomch"],
                ['text'=>$title2,'callback_data'=>"anomch"],
                ];
        }
    }
    $keys[] = [['text'=>"↩️ برگشت",'callback_data'=>"mainMenu"]];
...
if(preg_match('/decline(\d+)_(\d+)/',$userInfo['step'],$match) and $text != $cancelText){
    ...
    bot('editMessageReplyMarkup',[
		...
		'message_id' => $match[2],
		'reply_markup' => json_encode(['inline_keyboard'=>[
		    [['text'=>"لغو شد ❌",'callback_data'=>"anomch"]]
		    ]])
    ]);
...
if($data == 'dayPlanSettings' and ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keyboard = [];
    $keyboard[] = [['text'=>"حذف",'callback_data'=>"anomch"],['text'=>"قیمت",'callback_data'=>"anomch"],['text'=>"تعداد روز",'callback_data'=>"anomch"]];
...
if($data == 'volumePlanSettings' and ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keyboard = [];
    $keyboard[] = [['text'=>"حذف",'callback_data'=>"anomch"],['text'=>"قیمت",'callback_data'=>"anomch"],['text'=>"مقدار حجم",'callback_data'=>"anomch"]];
...
if(preg_match('/^closeTicket_(\d+)/',$data,$match) and  $from_id != $admin){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ['text'=>"$from_id",'callback_data'=>"anomch"],
            ['text'=>"آیدی کاربر",'callback_data'=>'anomch']
        ],
        [
            ['text'=>$first_name??" ",'callback_data'=>"anomch"],
            ['text'=>"اسم کاربر",'callback_data'=>'anomch']
        ],
        [
            ['text'=>"$title",'callback_data'=>'anomch'],
            ['text'=>"عنوان",'callback_data'=>'anomch']
        ],
        [
            ['text'=>"$category",'callback_data'=>'anomch'],
            ['text'=>"دسته بندی",'callback_data'=>'anomch']
        ],
        ]]);
...
if(preg_match("/^rate_+([0-9])+_+([0-9])/",$data,$match)){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ['text'=>"رای تیکت",'callback_data'=>"anomch"]
            ],
        ]]);
...
if($data=='ticketsCategory' and ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $stmt->close();
    $keys = array();
    $keys[] = [['text'=>"حذف",'callback_data'=>"anomch"],['text'=>"دسته بندی",'callback_data'=>"anomch"]];
    
    if($ticketCategory->num_rows>0){
        while($row = $ticketCategory->fetch_assoc()){
            $rowId = $row['id'];
            $ticketName = $row['value'];
            $keys[] = [['text'=>"❌",'callback_data'=>"delTicketCat_$rowId"],['text'=>$ticketName,'callback_data'=>"anomch"]];
        }
    }else{
...
if(preg_match('/^closeTicket_(\d+)/',$data,$match) and  ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    bot('editMessageReplyMarkup',['chat_id'=>$from_id,'message_id'=>$message_id,'reply_markup'=>json_encode(['inline_keyboard'=>[
        [['text'=>"تیکت بسته شد",'callback_data'=>"anomch"]]
        ]])]);
    ...
    while($row = $serversList->fetch_assoc()){
        ...
        if($response->success){
            ...
            if(strpos($list, $text)){
                ...
                $keys = json_encode(['inline_keyboard'=>[
                [
                    ['text'=>$state??" ",'callback_data'=>"anomch"],
                    ['text'=>"🔘 وضعیت اکانت 🔘",'callback_data'=>"anomch"],
                    ],
                [
					['text'=>$remark??" ",'callback_data'=>"anomch"],
                    ['text'=>"« نام اکانت »",'callback_data'=>"anomch"],
                    ],
                [
                    ['text'=>$upload?? " ",'callback_data'=>"anomch"],
                    ['text'=>"√ آپلود √",'callback_data'=>"anomch"],
                    ],
                [
                    ['text'=>$download??" ",'callback_data'=>"anomch"],
                    ['text'=>"√ دانلود √",'callback_data'=>"anomch"],
                    ],
                [
                    ['text'=>$total??" ",'callback_data'=>"anomch"],
                    ['text'=>"† حجم کلی †",'callback_data'=>"anomch"],
                    ],
                [
                    ['text'=>$leftMb??" ",'callback_data'=>"anomch"],
                    ['text'=>"~ حجم باقیمانده ~",'callback_data'=>"anomch"],
                    ],
                [
                    ['text'=>$expiryTime??" ",'callback_data'=>"anomch"],
                    ['text'=>"تاریخ اتمام",'callback_data'=>"anomch"],
                    ],
                [
                    ['text'=>$expiryDay??" ",'callback_data'=>"anomch"],
                    ['text'=>"تعداد روز باقیمانده",'callback_data'=>"anomch"],
                    ],
...
if($data == 'backplan' and ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keyboard[] = [['text'=>"➖➖➖",'callback_data'=>"anomch"]];
...
if(($data=="editCustomPlan" || preg_match('/^editCustom(gbPrice|dayPrice)/',$userInfo['step'],$match)) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $cancelText){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ...
            ['text'=>"هزینه هر گیگ",'callback_data'=>"anomch"]
            ],
        [
            ['text'=>$dayPrice,'callback_data'=>"editCustomdayPrice"],
            ['text'=>"هزینه هر روز",'callback_data'=>"anomch"]
            ],
        [
            ['text'=>"برگشت 🔙",'callback_data'=>"backplan"]
            ]
...
if(preg_match('/^anomplanacclist(\d+)/',$data,$match) and ($from_id == $admin || $userInfo['isAdmin'] == true)){
...
if(preg_match('/^anomplandelete(\d+)/',$data,$match) and ($from_id == $admin || $userInfo['isAdmin'] == true)){
...
if(preg_match('/^anomplanname(\d+)/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $cancelText){
    setUser($data);
    delMessage();
    sendMessage("🔅 یه اسم برا پلن جدید انتخاب کن:",$cancelKey);exit;
}
if(preg_match('/^anomplanname(\d+)/',$userInfo['step'], $match) && $text != $cancelText){
...
if(preg_match('/^anomplanslimit(\d+)/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $cancelText){
    setUser($data);
    delMessage();
    sendMessage("🔅 ظرفیت جدید برای پلن انتخاب کن:",$cancelKey);exit;
}
if(preg_match('/^anomplanslimit(\d+)/',$userInfo['step'], $match) && $text != $cancelText){
...
if(preg_match('/^anomplansinobundid(\d+)/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $cancelText){
    setUser($data);
    delMessage();
    sendMessage("🔅 سطر جدید برای پلن انتخاب کن:",$cancelKey);exit;
}
if(preg_match('/^anomplansinobundid(\d+)/',$userInfo['step'], $match) && $text != $cancelText){
...
if(preg_match('/^anomplaneditdes(\d+)/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $cancelText){
    setUser($data);
    delMessage();
    sendMessage("🎯 توضیحاتت رو برام وارد کن:",$cancelKey);exit;
}
if(preg_match('/^anomplaneditdes(\d+)/',$userInfo['step'], $match) && $text != $cancelText){
...
if(preg_match('/^anomplanrial(\d+)/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $cancelText){
    setUser($data);
    delMessage();
    sendMessage("🎯 شیطون قیمت و گرون کردی 😂 ، خب قیمت جدید و بزن ببینم :",$cancelKey);exit;
}
if(preg_match('/^anomplanrial(\d+)/',$userInfo['step'], $match) && $text != $cancelText){
    if(is_numeric($text)){
...
if(preg_match('/^anomcategorydelete(\d+)_(\d+)/',$data, $match) and ($from_id == $admin || $userInfo['isAdmin'] == true)){
...
if(preg_match('/^anomcategoryedit/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $cancelText){
    setUser($data);
    delMessage();
    sendMessage("〽️ یه اسم جدید برا دسته بندی انتخاب کن:",$cancelKey);exit;
}
if(preg_match('/anomcategoryedit(\d+)_(\d+)/',$userInfo['step'], $match) && $text != $cancelText){
...
if(preg_match('/^anomdeleteserver(\d+)/',$data,$match) and ($from_id == $admin || $userInfo['isAdmin'] == true)){
