...
function getServerConfigKeys($serverId,$offset = 0){
    ...
    return json_encode(['inline_keyboard'=>[
        [
            ['text'=>$panelUrl,'callback_data'=>"anomch"],
            ],
        [
            ['text'=>$cname,'callback_data'=>"editServerName$id"],
            ['text'=>"❕نام سرور",'callback_data'=>"anomch"]
            ],
        [
            ['text'=>$flaganom,'callback_data'=>"editServerFlag$id"],
            ['text'=>"🚩 پرچم سرور",'callback_data'=>"anomch"]
            ],
        [
            ['text'=>$remarkanom,'callback_data'=>"editServerRemark$id"],
            ['text'=>"📣 ریمارک سرور",'callback_data'=>"anomch"]
            ],
        [
            ['text'=>$serverType??" ",'callback_data'=>"changeServerType$id"],
            ['text'=>"🔅نوعیت سرور",'callback_data'=>"anomch"]
            ],
        [
            ['text'=>$portType,'callback_data'=>"changePortType$id"],
            ['text'=>"🔅نوعیت پورت",'callback_data'=>"anomch"]
            ],
        [
            ['text'=>$ucount,'callback_data'=>"editServerMax$id"],
            ['text'=>"🔅ظرفیت سرور",'callback_data'=>"anomch"]
            ],
        [
            ['text'=>$sni,'callback_data'=>"editsServersni$id"],
            ['text'=>"sni",'callback_data'=>"anomch"],
            ],
        [
            ['text'=>$headerType,'callback_data'=>"editsServerheader_type$id"],
            ['text'=>"header type",'callback_data'=>"anomch"],
            ],
        [
            ['text'=>$requestHeader,'callback_data'=>"editsServerrequest_header$id"],
            ['text'=>"request header",'callback_data'=>"anomch"],
            ],
        [
            ['text'=>$responseHeader,'callback_data'=>"editsServerresponse_header$id"],
            ['text'=>"response header",'callback_data'=>"anomch"],
            ],
        [
            ['text'=>$security,'callback_data'=>"editsServersecurity$id"],
            ['text'=>"security",'callback_data'=>"anomch"],
            ],
        (($serverConfig['type'] == "sanaei" || $serverConfig['type'] == "alireza")?
        [
            ['text'=>$reality,'callback_data'=>"changeRealityState$id"],
            ['text'=>"reality",'callback_data'=>"anomch"],
            ]:[]),
        [
        ...
        [
            ['text'=>"✂️ حذف سرور",'callback_data'=>"anomdeleteserver$id"],
            ],
...
function getServerListKeys($offset = 0){
    ...
    $keys = array();
    $keys[] = [['text'=>"وضعیت",'callback_data'=>"anomch"],['text'=>"تنظیمات",'callback_data'=>"anomch"],['text'=>"نوعیت",'callback_data'=>"anomch"],['text'=>"سرور",'callback_data'=>"anomch"]];
    if($cats->num_rows == 0){
        $keys[] = [['text'=>"سروری یافت نشد",'callback_data'=>"anomch"]];
    }else {
        while($cty = $cats->fetch_assoc()){
            ...
            $keys[] = [['text'=>$state,'callback_data'=>'toggleServerState' . $id . "_" . $offset],['text'=>"⚙️",'callback_data'=>"showServerSettings" . $id . "_" . $offset],['text'=>$serverType??" ",'callback_data'=>"anomch"],['text'=>$cname,'callback_data'=>"anomch"]];
        } 
    }
...
function getCategoriesKeys($offset = 0){
    ...
    $keys = array();
    $keys[] = [['text'=>"حذف",'callback_data'=>"anomch"],['text'=>"اسم دسته",'callback_data'=>"anomch"]];
    if($cats->num_rows == 0){
        $keys[] = [['text'=>"دسته بندی یافت نشد",'callback_data'=>"anomch"]];
    }else {
        while($cty = $cats->fetch_assoc()){
            $id = $cty['id'];
            $cname = $cty['title'];
            $keys[] = [['text'=>"❌",'callback_data'=>"anomcategorydelete$id" . "_" . $offset],['text'=>$cname,'callback_data'=>"anomcategoryedit$id" . "_" . $offset]];
        }
    }
...
function getBotReportKeys(){
    ...
    return json_encode(['inline_keyboard'=>[
        [
            ['text'=>$allUsers,'callback_data'=>'anomch'],
            ['text'=>"تعداد کل کاربران",'callback_data'=>'anomch']
            ],
        [
            ['text'=>$allOrders,'callback_data'=>'anomch'],
            ['text'=>"کل محصولات خریداری شده",'callback_data'=>'anomch']
            ],
        [
            ['text'=>$allServers,'callback_data'=>'anomch'],
            ['text'=>"تعداد سرورها",'callback_data'=>'anomch']
            ],
        [
            ['text'=>$allCategories,'callback_data'=>'anomch'],
            ['text'=>"تعداد دسته ها",'callback_data'=>'anomch']
            ],
        [
            ['text'=>$allPlans,'callback_data'=>'anomch'],
            ['text'=>"تعداد پلن ها",'callback_data'=>'anomch']
            ],
        [
            ['text'=>$totalRewards,'callback_data'=>'anomch'],
            ['text'=>"درآمد کل",'callback_data'=>'anomch']
            ],
        [
...
function getAdminsKeys(){
    ...
    if($usersList->num_rows > 0){
        while($user = $usersList->fetch_assoc()){
            $keys[] = [['text'=>"❌",'callback_data'=>"delAdmin" . $user['userid']],['text'=>$user['name'], "callback_data"=>"anomch"]];
        }
    }else{
        $keys[] = [['text'=>"لیست ادمین ها خالی است ❕",'callback_data'=>"anomch"]];
    }
    ...
    if($userCount->num_rows > 0){
        ...
        return json_encode(['inline_keyboard'=>[
            [
                ...
                ['text'=>"یوزرنیم",'callback_data'=>"anomch"]
                ],
            [
                ['text'=>$fullName??" ",'callback_data'=>"anomch"],
                ['text'=>"نام",'callback_data'=>"anomch"]
                ],
            [
                ['text'=>$boughtService??" ",'callback_data'=>"anomch"],
                ['text'=>"سرویس ها",'callback_data'=>"anomch"]
                ],
            [
                ['text'=>$totalBoughtPrice??" ",'callback_data'=>"anomch"],
                ['text'=>"مبلغ خرید",'callback_data'=>"anomch"]
                ],
            [
                ['text'=>$userWallet??" ",'callback_data'=>"anomch"],
                ['text'=>"موجودی کیف پول",'callback_data'=>"anomch"]
                ],
            [
...
function getDiscountCodeKeys(){
    ...
    $keys = array();
    if($list->num_rows > 0){
        $keys[] = [['text'=>'حذف','callback_data'=>"anomch"],['text'=>"تاریخ ختم",'callback_data'=>"anomch"],['text'=>"تعداد استفاده",'callback_data'=>"anomch"],['text'=>"مقدار تخفیف",'callback_data'=>"anomch"],['text'=>"کد تخفیف",'callback_data'=>"anomch"]];
        while($row = $list->fetch_assoc()){
            ...
            $keys[] = [['text'=>'❌','callback_data'=>"delDiscount" . $rowId],['text'=>$date,'callback_data'=>"anomch"],['text'=>$count,'callback_data'=>"anomch"],['text'=>$amount,'callback_data'=>"anomch"],['text'=>$hashId,'callback_data'=>'copyHash' . $hashId]];
        }
    }else{
        $keys[] = [['text'=>"کد تخفیفی یافت نشد",'callback_data'=>"anomch"]];
    }
    ...
    if($buttons->num_rows > 0){
        while($row = $buttons->fetch_assoc()){
            ...
            $keys[] = [
                        ...
                        ['text'=>$title??" " ,'callback_data'=>"anomch"]];
        }
    }else{
        $keys[] = [['text'=>"دکمه ای یافت نشد ❕",'callback_data'=>"anomch"]];
    }
...
function getPlanDetailsKeys($planId){
    ...
    if($pdResult->num_rows == 0) return null;
        ...
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $anomplanaccnumber = $stmt->get_result()->num_rows;
        $stmt->close();

        $srvid= $pd['server_id'];
        $keyboard = [
            ($rahgozar==true?[['text'=>"* نوع پلن: رهگذر *",'callback_data'=>'anomch']]:[]),
            [['text'=>$name,'callback_data'=>"anomplanname$id"],['text'=>"🔮 نام پلن",'callback_data'=>"anomch"]],
            ($reality == "true"?[['text'=>$dest,'callback_data'=>"editDestName$id"],['text'=>"dest",'callback_data'=>"anomch"]]:[]),
            ($reality == "true"?[['text'=>$serverName,'callback_data'=>"editServerNames$id"],['text'=>"serverNames",'callback_data'=>"anomch"]]:[]),
            ($reality == "true"?[['text'=>$spiderX,'callback_data'=>"editSpiderX$id"],['text'=>"spiderX",'callback_data'=>"anomch"]]:[]),
            ($reality == "true"?[['text'=>$flow,'callback_data'=>"editFlow$id"],['text'=>"flow",'callback_data'=>"anomch"]]:[]),
            [['text'=>$anomplanaccnumber,'callback_data'=>"anomch"],['text'=>"🎗 تعداد اکانت های فروخته شده",'callback_data'=>"anomch"]],
            ($pd['inbound_id'] != 0?[['text'=>"$acount",'callback_data'=>"anomplanslimit$id"],['text'=>"🚪 تغییر ظرفیت کانفیگ",'callback_data'=>"anomch"]]:[]),
            ($pd['inbound_id'] != 0?[['text'=>$pd['inbound_id'],'callback_data'=>"anomplansinobundid$id"],['text'=>"🚪 سطر کانفیگ",'callback_data'=>"anomch"]]:[]),
            [['text'=>"✏️ ویرایش توضیحات",'callback_data'=>"anomplaneditdes$id"]],
            [['text'=>number_format($price) . " تومان",'callback_data'=>"anomplanrial$id"],['text'=>"💰 قیمت پلن",'callback_data'=>"anomch"]],
            [['text'=>"♻️ دریافت لیست اکانت ها",'callback_data'=>"anomplanacclist$id"]],
            [['text'=>"✂️ حذف",'callback_data'=>"anomplandelete$id"]],
        ...
        if($inbound_id == 0){
            if($protocol == 'trojan') {
                if($security == "xtls"){
                    $keyboard = [
                        [
            			    ['text' => "$name", 'callback_data' => "anomch"],
                            ['text' => " 🚀 نام پلن:", 'callback_data' => "anomch"],
                        ],
                        [
            			    ['text' => "$date ", 'callback_data' => "anomch"],
                            ['text' => "⏰  تاریخ خرید: ", 'callback_data' => "anomch"],
                        ],
                        [
            			    ['text' => "$expire_date ", 'callback_data' => "anomch"],
                            ['text' => "⏰  تاریخ انقضاء: ", 'callback_data' => "anomch"],
                        ],
                        [
            			    ['text' => " $leftgb", 'callback_data' => "anomch"],
                            ['text' => "⏳ حجم باقیمانده:", 'callback_data' => "anomch"],
            			],
            // 			[
            //                 ['text' => $netType. " 🎛 نوع شبکه ", 'callback_data' => "cantEditTrojan"],
            //             ],
                        [
                            ['text' => "🚦 پروتکل انتخابی", 'callback_data' => "anomch"],
                        ],
                        [
            ...
            $keyboard = [
                ...
                [
                    ['text' => " $protocol پروتکل ☑️", 'callback_data' => "anomch"],
                ]
            ];
